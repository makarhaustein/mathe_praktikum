...
bool GeneratePrime::check_prime(vector<int> mult, vector<int> primes_array, int ord, int current){
  int n = 2; 
  bool is_prime_current = true;
  while (n < ord && is_prime_current) { 
    while (mult[n] < current){
      mult[n] += primes_array[n] + primes_array[n]; 
    }
    if (mult[n] == current) {
